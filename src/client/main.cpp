...
int main(int argc, char **argv)
{
    ...
    {
        cerr << "connect chatserver error" << endl;
        close(clientfd);
        exit(-1);
    }
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        //接收ChatServer转发的数据，反序列化成json数据对象
        ...
        {
            doLoginResponse(js); //处理登陆响应的业务处理逻辑
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning)
    {
        ...
        //调用相应命令的处理回调，mianmenu对修改关闭，添加新功能不需要修改该函数
...
void showCurrentUserData()
{
    ...
    if (!g_currentUserGroupList.empty())
    {
        ...
        {
            cout << "群id:" << group.getId() << " "
                 << "群名:" << group.getName() << " "
                 << "群描述:" << group.getDesc() << endl;
            for (GroupUser &user : group.getUsers())
            {
                cout << "id:" << user.getId() << " "
                     << "昵称:" << user.getName() << " "
                     << "状态:" << user.getState() << " "
                     << "角色:" << user.getRole() << endl;
            }
        }
    }
    cout << "==================================================================================" << endl;
